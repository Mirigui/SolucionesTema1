...
#b)
#Primera vision de mammals
dim(mammals)
head(mammals)
tail(mammals)
str(mammals)

#Primera vision de Animals2
dim(Animals2)
head(Animals2)
tail(Animals2)
str(Animals2)
...
```{r}
#La funcion setdiff no funciona igual para ambos lados por tanto:
mammals_primero <- setdiff(mammals, Animals2)
mammals_primero

Animals2_primero <- setdiff(Animals2,mammals)
Animals2_primero

#En el primer caso todos los animales que hay en mammals aparecen tambien en Animals2, por tanto hay 0

#En el segundo caso si hay 3v animales que se encuentran en Animals2 pero que no estan en mammlas ya que se tratan de 3 dinosaurios, por tanto reptiles.
...
```{r}
#La diferencia viene dada como ya he dicho anteriormente, como bien su nombre indica en el conjunto mammals todos loa animlaes son mamiferos y en cambio en el conjunto Animals2 tambien hay 3 reptiles(dinosaurios) por tanto no iban a esatr en el otro conjunto.
...
```{r}
#Hacemos el de mammals
qqPlot(log(mammals$brain),col = "red")

#Hacemos el de Animals2
qqPlot(log(Animals2$brain),col = "red")

#Presentan un comportamiento bastante parecido, yo creo q sobretodo en el de Animals 2 si se podria decir que sigue una distribucion gaussiana
...
#Tiene 60 registros con 4 variables: 2 de tipo factor, 1 tipo numeric y 3 tipo integer

#b)
colSums(is.na(cabbages))
#Como se puede ver todos los elementos = FALSE por tanto no hay elementos NA

#c)
dim(Chile)
lapply(Chile,class)

#Tiene 2700 registros con 8 variables: 4 de tipo factor, 1 tipo numeric y otro tipo integer


colSums(is.na(Chile))
#Hay un valor en sex,11 en education,98 en income,17 en statusquo y 168 en vote

#d)
summary(cabbages)
summary(Chile)
#Ya estria el ejercicio 4
```
...
#Sobretodo en el caso de HeadWt , pero yo creo que ambas variables verifican la condicion
...
#Si hay diferencia, para el cult c39 , sulen tenem menos vitC que el c52 
...
#Diagrama de dispersión etiquetando según un factor.
plot(cabbages$HeadWt, cabbages$VitC, pch=c(6,16)[cabbages$HCult])
legend(x="topright",legend=c("Cult = c39","Cult= c52"), pch=c(6,16))


#Se puede ver la relacion de que cuanto mas pesa la calabaza menos vitamina C contiene
